...
| R2000 DXF AutoCAD Open path + DWG this app reopens; native HATCH with islands, DIMENSION, MTEXT, SPLINE; every entity type has a stated, tested round trip | ODA-grade DWG every AutoCAD opens without Recover |
...
| `RENDER 1920 [PLACE] [LEVEL]` | High-res PNG; LEVEL keeps verticals vertical | `VIEW SAVE HERO` / `VIEW HERO` | Named cameras, saved with the project |
| `TURNTABLE 8` | One-orbit webm of the model | `VIEWS` | List the saved cameras |
| `WALK 12` | Flythrough of the saved views, to webm | `WALK A C 8` | Same, named stops in order |
...
| `KEYMAP ACAD` | AutoCAD keys: E erase, M move, U undo, X explode | `KEYMAP SD` | The native map |

A first visit walks the sample cabin through MODEL and ROOF once, skippable and undoable, so the building pipeline introduces itself. The tool rail shows one row at a time behind Draw / Modify / Issue / Build tabs; the tab you leave it on survives reloads, and picking a tool by key or command pulls its row forward. In 3D the Build row is pinned.
...
- **Click** a solid to select it by name. **Drag** moves it in plan, shift lifts it. The drag infers against the real geometry of every other solid: corners lock corner-to-corner and edge midpoints lock the same way, exactly, with the HUD naming the inference (vertex, midpoint), so a box corner lands on a roof ridge end or an off-grid CSG corner without typing a number. Where no point is near it falls back to face-and-centre alignment and the half-foot grid (alt frees it entirely). The live delta reads out in feet and inches; **type a number, Enter** sets the distance exactly. **Ctrl-drag** commits a copy.
...
**Sun, materials, renders.** `SUN JUN 21 14 40.7` puts the real sun in the sky: azimuth and elevation from the standard declination and hour-angle formulas, held to almanac values in the tests (solstice noon at 40N, due south at solar noon, exactly on the horizon at equinox six o'clock). The model throws true shadows on itself and the ground, which makes the 3D view a shadow study, the thing planning boards ask for. `MAT ROOF #7a3b2a 0.85` paints a solid, its stacked levels, a layer or a kind with PBR colour, roughness and metalness; materials and the sun are document data, saved with the project and one undo away. `RENDER 1920` draws the scene offscreen at print resolution and downloads a PNG; `RENDER 1920 PLACE` sets the rendering beside the drawing as an image entity, the perspective a real set opens with, and it prints: placed renders and IMG underlays enter the document as baseline JPEG and the PDF writer embeds each unique one verbatim as a DCTDecode image object, drawn at true position, size and rotation. DXF keeps an honest dashed frame, since a data URL has no file for an IMAGEDEF to reference. The saved project file always carries every image; the autosave and share link carry what fits (each image up to ~220 KB, ~1.9 MB together, largest dropped first), and a stripped image keeps a labeled frame on the canvas plus a toast saying where its pixels live. This is a raster study renderer, not Cycles: no path tracing, no textures, stated plainly. `VIEW SAVE HERO` names the current 3D camera and stores it in the project file; `VIEW HERO` walks back to it exactly, `VIEWS` lists them. `RENDER 1600 LEVEL` levels the camera and recovers the framing with a film offset, the two point perspective of an architectural rendering where verticals stay vertical. `TURNTABLE 8` records one full orbit of the model to a webm, straight off the canvas. `WALK 12` rides a Catmull-Rom spline through the saved views in order (or `WALK A C 8` through named stops), eased at both ends, and records the flythrough the same way; the spline passes through every saved camera exactly and reproduces a straight dolly exactly, both held in the tests.
...
Shipped: kernel + CLI + embed, honest AI, Grok drafting with no key, R2000 DXF AutoCAD Open path (island hatches round-trip both ways), DWG this app reopens, paperspace kept on open, associative dims that follow a stretch, issued 24×36 PDF with ISO lineweights and embedded fonts, cutting-plane sections, isolated details, feature control frames, ± tolerances. Mesh 3D on an exact CSG kernel: primitives, extrude / revolve / loft / sweep, push-pull face editing, edit mode (E, Tab, or the EDIT button on the 3D rail, then grab any vertex, edge or face of a solid and drag it, snapped and typed), precision touch (vertex and midpoint inference snapping, typed distances, rotate, copy), measure, STL both ways, OBJ/glTF out. The building pipeline: MODEL, STACK, wing roofs with valleys, dormers, per-storey plans, roof plan, hidden-line elevations, sections that see beyond the cut, DRAWINGS SHEETS, QTO. The archviz layer: almanac-tested SUN with true shadows, MAT materials, RENDER to print-resolution PNG or placed on the drawing, saved named cameras, two-point perspective, TURNTABLE orbit video, WALK flythroughs along a spline of saved views. Starters: cabin, plate, GA, 3D bracket.
...
- `npm run job`: the one real job, timed. Walls, openings, rooms, MODEL, ROOF, DORMER, DRAWINGS, Export PDF, driven through the built app by pointer, chip and menu the way a person does it. Any step that changes nothing and says nothing fails the run.
