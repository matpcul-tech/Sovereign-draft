 ...
 */
import { dist, distToSeg, arcPoints, dimGeom, pointInPoly } from './geometry.js';
import { hatchLines } from './hatch.js';
import { expandInsert, insertGrips } from './dynblock.js';

export function flattenEnt(e){
  if (e && e.type === 'insert') return expandInsert(e);
  return e ? [e] : [];
}
...
export function entPoints(e){
  ...
  if (e.type === 'line'){ p.push([e.x1, e.y1, 0], [e.x2, e.y2, 0], [(e.x1 + e.x2) / 2, (e.y1 + e.y2) / 2, 1]); }
    ...
    for (let i = 0; i < pts.length; i++){
      ...
      let j = i + 1; if (j === pts.length){ if (!(e.closed || e.type === 'hatch')) break; j = 0; }
...
export function translateEnt(e, dx, dy){
  ...
  else if (e.type === 'poly' || e.type === 'hatch'){ for (let i = 0; i < e.pts.length; i++){ e.pts[i][0] += dx; e.pts[i][1] += dy; } }
...
export function entBBox(e, bb){
  ...
  else if (e.type === 'poly' || e.type === 'hatch'){ for (let i = 0; i < e.pts.length; i++) add(e.pts[i][0], e.pts[i][1]); }
...
export function rotateMembers(ms){
  ...
  ms.forEach(e => {
    ...
    else if (e.type === 'poly' || e.type === 'hatch'){ for (let i = 0; i < e.pts.length; i++){ p = rot(e.pts[i][0], e.pts[i][1]); e.pts[i] = [p[0], p[1]]; } }
