/* Hand-rolled single-page PDF (Letter landscape) printed at a true
 * architectural scale, with title block and scale bar. Returns the PDF source
 * as a string; the caller downloads it.
 */
...
export function pdfSafe(s){
  s = String(s).replace(/½/g, ' 1/2').replace(/×/g, 'x').replace(/Δ/g, 'd').replace(/·/g, '-').replace(/°/g, ' deg');
  let out = '';
...
/* opts: { ppf: number | 'fit', layerVisible(name)=>bool, projectName, dateStr } */
export function buildPDF(entities, opts){
  ...
  visible.forEach(e => {
    ...
    P((e.layer === 'WALLS' ? '1.4' : (isDim ? '0.4' : '0.7')) + ' w');
    P((isDim ? '0.35' : '0.08') + ' G');
    if (e.type === 'line') seg(e.x1, e.y1, e.x2, e.y2);
      ...
      const txt = fmtFtIn(g.len), sz = 7.5;
